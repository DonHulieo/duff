--[[local function GetAllEvents()
    ...
    for k, v in pairs(_G) do
        if type(v) == 'table' and v.name ~= nil and v.name:sub(1, 7) == 'event__' then
...
AddEventHandler('gameEventTriggered', function(name, args)
    print('Game Event Triggered: ' .. name .. ' ' .. json.encode(args))
end)
...
CreateThread(function()
    while true do
        ...
        for index, event in pairs(events) do
            ...
            if IsShockingEventInSphere(event, coords.x, coords.y, coords.z, 100.0) then
                print(event)
            end
        end
    end
