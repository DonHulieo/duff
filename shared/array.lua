...
local array do
  ...
  local function push(list, arg, ...)
    if not is_array(list) then trace('error', 'bad argument #1 to \'push\' (array expected, got '..type(list)..')') return end
  ...
  local function push_array(list, tbl)
    if not is_array(list) then trace('error', 'bad argument #1 to \'pusharray\' (array expected, got '..type(list)..')') return end
    if not is_array(tbl) then trace('error', 'bad argument #2 to \'pusharray\' (array expected, got '..type(tbl)..')') return end
  ...
  local function peek(list, index)
    if not is_array(list) then trace('error', 'bad argument #1 to \'peek\' (array expected, got '..type(list)..')') return end
    index = index or 1
    local len = #list
  ...
  local function peek_array(list, index)
    if not is_array(list) then trace('error', 'bad argument #1 to \'peekarray\' (array expected, got '..type(list)..')') return end
    index = index or 1
    local len, res = #list, {}
  ...
  local function pop(list, index)
    if not is_array(list) then trace('error', 'bad argument #1 to \'pop\' (array expected, got '..type(list)..')') return end
    index = index or 1
  ...
  local function pop_array(list, index)
    if not is_array(list) then trace('error', 'bad argument #1 to \'poparray\' (array expected, got '..type(list)..')') return end
    index = index or 1
    local res = {}
  ...
  local function contains(list, key, value)
    if not is_array(list) then trace('error', 'bad argument #1 to \'contains\' (array expected, got '..type(list)..')') return end
    if not key and not value then trace('warn', 'bad argument #2 to \'contains\' (key or value expected, got nil)') return end
  ...
  local function copy(list)
    if not is_array(list) then trace('error', 'bad argument #1 to \'copy\' (array expected, got '..type(list)..')') return end
    local res = {}
  ...
  local function find(list, fn)
    if not is_array(list) then trace('error', 'bad argument #1 to \'find\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'find\' (function expected, got '..type(fn)..')') return end
    for i = 1, #list do
  ...
  local function fold_left(list, fn, arg)
    if not is_array(list) then trace('error', 'bad argument #1 to \'foldleft\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'foldleft\' (function expected, got '..type(fn)..')') return end
    local res = list[1]
  ...
  local function fold_right(list, fn, arg)
    if not is_array(list) then trace('error', 'bad argument #1 to \'foldright\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'foldright\' (function expected, got '..type(fn)..')') return end
    local res = list[#list]
    res = arg and fn(res, arg) or res
  ...
  local function map(list, fn, in_place)
    if not is_array(list) then trace('error', 'bad argument #1 to \'map\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'map\' (function expected, got '..type(fn)..')') return end
  ...
  local function filter(list, fn, in_place)
    if not is_array(list) then trace('error', 'bad argument #1 to \'filter\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'filter\' (function expected, got '..type(fn)..')') return end
    if in_place then
      local i = 1
      while i <= #list do
  ...
  local function for_each(list, fn, reverse)
    if not is_array(list) then trace('error', 'bad argument #1 to \'foreach\' (array expected, got '..type(list)..')') return end
    if not fn then trace('error', 'bad argument #2 to \'foreach\' (function expected, got '..type(fn)..')') return end
  ...
  local function reverse(list, length)
    if not is_array(list) then trace('error', 'bad argument #1 to \'reverse\' (array expected, got '..type(list)..')') return end
