...
local math do
  ...
  local function between(val, min, max)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'between\' (number expected, got '..type(val)..')') return end
    if not min or type(min) ~= 'number' then trace('error', 'bad argument #2 to \'between\' (number expected, got '..type(min)..')') return end
    if not max or type(max) ~= 'number' then trace('error', 'bad argument #3 to \'between\' (number expected, got '..type(max)..')') return end
  ...
  local function clamp(val, min, max)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'clamp\' (number expected, got '..type(val)..')') return end
    if not min or type(min) ~= 'number' then trace('error', 'bad argument #2 to \'clamp\' (number expected, got '..type(min)..')') return end
    if not max or type(max) ~= 'number' then trace('error', 'bad argument #3 to \'clamp\' (number expected, got '..type(max)..')') return end
  ...
  local function is_integer(val)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'isint\' (number expected, got '..type(val)..')') return end
    return val % 1 == 0
  end
  ...
  local function g_c_d(num, den)
    if not num or not is_integer(num) then trace('error', 'bad argument #1 to \'gcd\' (integer expected, got '..type(num)..')') return end
    if not den or not is_integer(den) then trace('error', 'bad argument #2 to \'gcd\' (integer expected, got '..type(den)..')') return end
    while den ~= 0 do
      num, den = den, num % den
    end
    return num
  ...
  local function is_half(val)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'ishalf\' (number expected, got '..type(val)..')') return end
    return val % 1 == .5
  end
  ...
  local function sign(val)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'sign\' (number expected, got '..type(val)..')') return end
  ...
  local function to_int(val) -- Converts a `float` to `integer` following the round half away from zero rule. <br> eg. `1.5 -> 2`, `-1.5 -> -2` <br> [Rounding Half Toward Zero](https://en.wikipedia.org/wiki/Rounding#Rounding_half_toward_zero)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'toint\' (number expected, got '..type(val)..')') return end
  ...
  local function round(val, increment)
    if not val or type(val) ~= 'number' then trace('error', 'bad argument #1 to \'round\' (number expected, got '..type(val)..')') return end
    if increment and (type(increment) ~= 'number' or increment < 1) then trace('error', 'bad argument #2 to \'round\' (number greater than 0 expected, got '..type(increment)..')') return end
  ...
  local function random(m, n)
    if not m or not is_integer(m) then trace('error', 'bad argument #1 to \'random\' (integer expected, got '..type(m)..')') return end
    if n and not is_integer(n) then trace('error', 'bad argument #2 to \'random\' (integer expected, got '..type(n)..')') return end
  ...
  local function timer(time, limit)
    if not time or not is_integer(time) then trace('error', 'bad argument #1 to \'timer\' (integer expected, got '..type(time)..')') return end
    if not limit or not is_integer(limit) then trace('error', 'bad argument #2 to \'timer\' (integer expected, got '..type(limit)..')') return end
  ...
  local function to_float(num, den) -- Converts an integer to float if `den` is not provided. <br> Converts a rational number to a float if `den` is provided.
    if not num or not is_integer(num) then trace('error', 'bad argument #1 to \'tofloat\' (integer expected, got '..type(num)..')') return end
    if den and not is_integer(den) then trace('error', 'bad argument #2 to \'tofloat\' (integer expected, got '..type(den)..')') return end
  ...
  local function to_rational(float, precision)
    if not float or type(float) ~= 'number' then trace('error', 'bad argument #1 to \'toratio\' (number expected, got '..type(float)..')') return end
    if precision and (type(precision) ~= 'number' or precision < 1e-10) then trace('error', 'bad argument #2 to \'toratio\' (number greater than 1e-10 expected, got '..type(precision)..')') return end
